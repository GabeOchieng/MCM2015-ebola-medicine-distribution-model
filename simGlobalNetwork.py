...
class Person:
    ...
    def chanceToContact(self, n, gridStdDev): #Chance to contact another node
        dist = math.sqrt((self.location[0]-n.location[0])**2 + (self.location[1]-n.location[1])**2)
        return math.exp(-0.5*(dist/(self.fear*gridStdDev[0])**2))    #Depends on gridStdDev and fear factor

    def chanceToInfect(self):  #Chance to infect contacted node
        return (1+math.exp(-0.46*(self.timeInfected-13.3)))**(-1)
...
class Contact:
    ...
    def __init__(self, n1, n2, timestep):
        self.distance = math.sqrt((n1.location[0]-n2.location[0])**2 + (n1.location[1]-n2.location[1])**2)
